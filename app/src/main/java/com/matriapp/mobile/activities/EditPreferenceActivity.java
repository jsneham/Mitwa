...
public class EditPreferenceActivity extends AppCompatActivity implements View.OnClickListener, SpinnerListener {
    ...
    private RelativeLayout lay_basic, lay_reli, lay_edu, lay_loca;
    ...
    private MultiSpinnerSearch spin_looking, spin_complx, spin_body_type, spin_eat, spin_smok, spin_drink, spin_mtongue,
            ...
            spin_emp_in, spin_occupation, spin_star;
...
//                            spin_designation.setSelection(desi_id);
    ...
    private void validBasicData() {
        ...
        if (!isValidId(fage_id)) {
            common.spinnerSetError(spin_age_from, "Please select minimum age");
            isValid = false;
        }
        if (!isValidId(tage_id)) {
            common.spinnerSetError(spin_age_to, "Please select maximum age");
            isValid = false;
        }
        if (!isValidId(fhite_id)) {
            common.spinnerSetError(spin_height_from, "Please select minimum height");
            isValid = false;
        }
        if (!isValidId(thite_id)) {
            common.spinnerSetError(spin_height_to, "Please select maximum height");
            isValid = false;
        }
